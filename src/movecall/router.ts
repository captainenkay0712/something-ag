...
const defaultAggregatorPublishedAt = Constants.AGGREGATOR_CONFIG.DEFAULT_PUBLISHED_AT.Mainnet
 ...
 */
const getAggregatorPublishedAt = (
    packages?: Map<string, string>,
    aggregatorPublishedAt?: string
): string => {
    // if (packages && packages.has(Constants.PACKAGE_NAMES.AGGREGATOR)) {
    //     return packages.get(Constants.PACKAGE_NAMES.AGGREGATOR)!
    // }
    // return aggregatorPublishedAt || defaultAggregatorPublishedAt
    return defaultAggregatorPublishedAt;
}

const newSwapContext = (
    params: SwapContext,
    txb: Transaction
): TransactionObjectArgument => {
    const {
...
interface TransferBalanceContext {
...
const transferBalance = (
    ...
    const { balance, coinType, recipient, aggregatorPublishedAt, packages } = params
    const publishedAt = getAggregatorPublishedAt(packages, aggregatorPublishedAt)
...
interface TransferOrDestroyCoinContext {
...
export {
    SwapContext,
    getAggregatorPublishedAt,
    newSwapContext,
    ConfirmSwapContext,
    confirmSwap,
    TakeBalanceContext,
    takeBalance,
    TransferBalanceContext,
    transferBalance,
    TransferOrDestroyCoinContext,
